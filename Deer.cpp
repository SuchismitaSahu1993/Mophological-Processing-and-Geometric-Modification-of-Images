...
unsigned char check_and_fill(int i, int j, unsigned char filter[9], unsigned char image[693][552][1]) {
    int count = 0;
...
int main(int argc, char *argv[]) {
    ...
    unsigned char Imagedata[length][width][1];
    ...
    for(int i=1; i < length+1; i++){
        for(int j=1; j < width+1; j++){
            OutputImage[i-1][j-1][0] = check_and_fill(i, j, filter, NewImagedataThree);
        }
    }
    ...
    fwrite(OutputImageFinal, sizeof(unsigned char), length*width*1, file);
    fclose(file);
